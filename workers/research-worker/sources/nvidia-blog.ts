...
export async function fetchNVIDIABlogSignals(): Promise<RawSignal[]> {
  ...
  try {
    ...
    for (const item of items.slice(0, researchConfig.maxSignalsPerSource)) {
      ...
      const keywords = ["AI", "GPU", "CUDA", "LLM", "deep learning", "neural", "accelerated", "inference"];
    ...
    console.error("NVIDIA Blog fetch error:", error);
  }

  return signals;
}

export const nvidiaBlogSource: ResearchSource = {
  name: "nvidia_blog",
  displayName: "NVIDIA Developer Blog",
  fetch: fetchNVIDIABlogSignals,
};
